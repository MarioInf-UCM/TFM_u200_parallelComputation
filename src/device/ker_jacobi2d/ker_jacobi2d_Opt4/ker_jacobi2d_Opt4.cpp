...
extern "C"{
    ...
    void innerLoop1(typeData *inD_A_local, typeData *inD_B_local, typeData *outD_result_local, int i){
        ...
        for (int j = 1; j < SIZE_N - 1 ; j++){
            ...
            #pragma HLS UNROLL factor=2
            inD_B_local[(i*SIZE_N)+j] = value * (inD_A_local[(i*SIZE_N)+j] + inD_A_local[(i*SIZE_N)+(j-1)] + inD_A_local[(i*SIZE_N)+(j+1)] + inD_A_local[((i+1)*SIZE_N)+j] + inD_A_local[((i-1)*SIZE_N)+j]);
    ...
    void innerLoop2(typeData *inD_A_local, typeData *inD_B_local, typeData *outD_result_local, int i){
        ...
        for (int j = 1; j < SIZE_N - 1 ; j++){
            ...
            #pragma HLS LOOP_TRIPCOUNT min=SIZE_N-1 max=SIZE_N-1
            #pragma HLS UNROLL factor=2
            inD_A_local[(i*SIZE_N)+j] = value * (inD_B_local[(i*SIZE_N)+j] + inD_B_local[(i*SIZE_N)+(j-1)] + inD_B_local[(i*SIZE_N)+(j+1)] + inD_B_local[((i+1)*SIZE_N)+j] + inD_B_local[((i-1)*SIZE_N)+j]);
    ...
    void writeData(typeData *outD_result, typeData *outD_result_local) {
        for (int i = 0 ; i < SIZE_N*SIZE_N*2 ; i++) {
            ...
            #pragma HLS PIPELINE II=1
            #pragma HLS LOOP_TRIPCOUNT min=SIZE_N*SIZE_N*2 max=SIZE_N*SIZE_N*2
            #pragma HLS UNROLL factor=2
    ...
    //*************************************
    #ifdef MINI_DATASET
        void ker_jacobi2d_Opt4_mini(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #elif defined(SMALL_DATASET)
        void ker_jacobi2d_Opt4_small(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #elif defined(MEDIUM_DATASET)
        void ker_jacobi2d_Opt4_medium(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #elif defined(LARGE_DATASET)
        void ker_jacobi2d_Opt4_large(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #elif defined(EXTRALARGE_DATASET)
        void ker_jacobi2d_Opt4_extralarge(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #else
        void ker_jacobi2d_Opt4(typeData *inD_A, typeData *inD_B, typeData *outD_result)
    #endif
    {
