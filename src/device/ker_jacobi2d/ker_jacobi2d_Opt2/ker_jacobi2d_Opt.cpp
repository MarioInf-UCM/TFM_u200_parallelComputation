        ...
        for (int t=0; t<STEPS ; t++){
            ...
            for (int i = 1; i < SIZE_N - 1 ; i++){
                ...
                for (int j = 1; j < SIZE_N - 1 ; j++){
                    ...
                    inD_B[(i*SIZE_N)+j] = factor * (inD_A[(i*SIZE_N)+j] + inD_A[(i*SIZE_N)+(j-1)] + inD_A[(i*SIZE_N)+(j+1)] + inD_A[((i+1)*SIZE_N)+j] + inD_A[((i-1)*SIZE_N)+j]);
            ...
            for (int i = 1; i < SIZE_N - 1 ; i++){
                ...
                for (int j = 1; j < SIZE_N - 1 ; j++){
                    ...
                    #pragma HLS UNROLL factor=8
                    #pragma HLS PIPELINE II=2
                    inD_A[(i*SIZE_N)+j] = factor * (inD_B[(i*SIZE_N)+j] + inD_B[(i*SIZE_N)+(j-1)] + inD_B[(i*SIZE_N)+(j+1)] + inD_B[((i+1)*SIZE_N)+j] + inD_B[((i-1)*SIZE_N)+j]);
