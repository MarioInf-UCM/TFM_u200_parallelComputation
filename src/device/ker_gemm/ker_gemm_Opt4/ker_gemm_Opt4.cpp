...
extern "C"{
    ...
    void processData(typeData *inD_A_local, typeData *inD_B_local, typeData *inD_C_local, typeData *outD_result_local){
        ...
        for (int i=0 ; i<NI ; i++){
            for (int j=0; j<NJ; j++){
                outD_result_local[(i*NJ)+j] = inD_C_local[(i*NJ)+j] * beta;
                for (int k=0; k<NK; k++){
                    #pragma HLS PIPELINE II=1
    ...
    #ifdef MINI_DATASET
        void ker_gemm_Opt4_mini(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(SMALL_DATASET)
        void ker_gemm_Opt4_small(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(MEDIUM_DATASET)
        void ker_gemm_Opt4_medium(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(LARGE_DATASET)
        void ker_gemm_Opt4_large(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(EXTRALARGE_DATASET)
        void ker_gemm_Opt4_extralarge(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #else
        void ker_gemm_Opt4(typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #endif
    {
