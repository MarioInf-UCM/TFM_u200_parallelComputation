...
atomic<bool> GemmPerHost_Opt4::stop_thread(false);
double GemmPerHost_Opt4::sharedVariable=0.0f;
...
GemmPerHost_Opt4::GemmPerHost_Opt4(){}
GemmPerHost_Opt4::~GemmPerHost_Opt4(){}
//*************************************
// MAIN FUNCTION - START
//*************************************
bool GemmPerHost_Opt4::exec(Execution exec, vector<double>& resultsPerformance, vector<double>& resultsPower, FileWriter_service fileWriter_logFile, FileWriter_service fileWriter_statsFile){
    fileWriter_logFile.writeln("Executing host function \"GemmPerHost::gemmPerHost_Opt4_exec\". Execution configuration:\n" + exec.displayInfo("\t"));
    ...
    GemmPerKernel data = GemmPerKernel(GEMM_ALPHA, GEMM_BETA, SIZE_I, SIZE_J, SIZE_K);
    ...
    ker.setArg(2, sendBuff_C);
    ker.setArg(3, recvBuff_resultDevice);
    ...
    typeData_fixed *temp_C = (typeData_fixed *)q.enqueueMapBuffer(sendBuff_C,
                                                        CL_TRUE,
                                                        CL_MAP_WRITE,
                                                        0,
                                                        data.get_SIZE_I() * data.get_SIZE_J() * sizeof(typeData));
...
bool GemmPerHost_Opt4::initParameter(Execution exec, unsigned int &SIZE_I, unsigned int &SIZE_J, unsigned int &SIZE_K){
...
void GemmPerHost_Opt4::emsamble_dataToBuffers(GemmPerKernel& data, typeData_fixed *temp_A, typeData_fixed *temp_B, typeData_fixed *temp_C, typeData_fixed *temp_resultDevice){
...
void GemmPerHost_Opt4::emsamble_buffersToData(GemmPerKernel& data, typeData_fixed *temp_resultDevice){
...
bool GemmPerHost_Opt4::compareResults(GemmPerKernel& data){
    typeData_fixed tolerance = 0.1;
    typeData_fixed tempDataCPU, tempDataDevice;
    
    for (int i = 0; i < data.get_SIZE_I(); i++){
        for (int j = 0; j < data.get_SIZE_J(); j++){
            tempDataCPU = data.get_resultCPU()[i][j];
            tempDataDevice = data.get_resultDevice()[i][j];
            if( !((tempDataCPU > tempDataDevice ? tempDataCPU - tempDataDevice : tempDataDevice - tempDataCPU) < tolerance) ){
                //cout << i << "  " << j << "  " << fixed << setprecision(10) << data.get_resultCPU()[i][j] << "  " << fixed << setprecision(10) << data.get_resultDevice()[i][j] << endl;
                return false;
            }
        }
    }
...
float GemmPerHost_Opt4::searchPropertyValue(const string& texto, const string& subcadena) {
...
double GemmPerHost_Opt4::executeAndMeasure_CPU(GemmPerKernel& data, EventTimer &event){
    ...
    CPU_ZERO(&cpuset);
    CPU_SET(core, &cpuset);
    function<void()> func = [&data]() { data.kernel_gemmPer_CPU(); };
...
double GemmPerHost_Opt4::executeAndMeasure_CPUopt(GemmPerKernel& data, vector<double> &measureByPack, EventTimer &event){
...
void GemmPerHost_Opt4::threadFunction_DevicePowerMeasure() {
