...
#define STREAM_SIZE 1024 * INCREMENT_FACTOR_STREAM
...
extern "C"{
    ...
    //*************************************
    #ifdef MINI_DATASET
        void ker_studyCase3_c_mini(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #elif defined(SMALL_DATASET)
        void ker_studyCase3_c_small(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #elif defined(MEDIUM_DATASET)
        void ker_studyCase3_c_medium(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #elif defined(LARGE_DATASET)
        void ker_studyCase3_c_large(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #elif defined(EXTRALARGE_DATASET)
        void ker_studyCase3_c_extralarge(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #else
        void ker_studyCase3_c(typeData *inD_vA, typeData *inD_vB, typeData *outD_result)
    #endif
    {
        ...
        for (int i = 0; i < SIZE; i += STREAM_SIZE) {
            ...
            for (int j = 0; j < actualChunkSize; j++) {
                ...
                #pragma HLS UNROLL factor=8
                if(inD_vB_local[j]==0){
                    outD_result[i + j] = inD_vA_local[j];
                }else{
