...
extern "C"{
    ...
    #ifdef MINI_DATASET
        void ker_gemm_Opt1_mini(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(SMALL_DATASET)
        void ker_gemm_Opt1_small(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(MEDIUM_DATASET)
        void ker_gemm_Opt1_medium(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(LARGE_DATASET)
        void ker_gemm_Opt1_large(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #elif defined(EXTRALARGE_DATASET)
        void ker_gemm_Opt1_extralarge(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #else
        void ker_gemm_Opt1(typeData inD_alpha, typeData inD_beta, typeData *inD_A, typeData *inD_B, typeData *inD_C, typeData *outD_result)
    #endif
    {
        ...
        for (int i=0 ; i<NI ; i++){
            ...
            for (int j=0; j<NJ; j++){
                ...
                outD_result_local[(i*NJ)+j] = inD_C_local[(i*NJ)+j] * inD_beta_local;

                for (int k=0; k<NK; ++k){
                    #pragma HLS LOOP_TRIPCOUNT min=NK max=NK
                    #pragma HLS UNROLL factor=8
                    #pragma HLS PIPELINE II=1
                    outD_result_local[(i*NJ)+j] += inD_alpha_local * inD_A_local[(i*NK)+k] * inD_B_local[(k*NJ)+j];
                }
            }
        }
